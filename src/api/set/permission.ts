...
export const editPermissionApi = (data: any) => {
  return request.put({ url: '/permission/edit', data })
}

// 删除权限
export const deletePermissionApi = (id: number) => {
  return request.delete({ url: `/permission/del/${id}` })
}

// 获取接口列表
export const getPermissionApiListApi = () => {
  return request.get({ url: '/permission/apiList' })
}

// 获取权限列表
export const getPermissionApi = (params: any) => {
  return request.get({ url: '/permission/list', params })
}

// 绑定接口
export const bindPermissionApi = (data: any) => {
  return request.post({ url: `/permission/bindApi`, data })
}
